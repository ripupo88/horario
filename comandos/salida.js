...
let f_procesa_salida = async (message) => {
    try {
        ...
        if (res_confirma) {
            ...
            await doSalida(message, empleado, res_confirma, false);
        }
    } catch (err) {
        let indice3 = 0;
        for (let cada_id of mensajes_abiertos) {
...
async function doSalida(message, empleado, res_confirma, system) {
    let salida_fichada = await mongo.f_nueva_salida(
        moment.unix(message.date).toISOString(),
        empleado.id,
        res_confirma
    );
    salida_fichada.res.salida = moment.unix(message.date).toISOString();
    console.log(salida_fichada);
    let admin_empresa = await mongo.f_obten_admin(empleado.id);

...
mongoose.connect(
    "mongodb://127.0.0.1/horariodb", { useNewUrlParser: true },
    err => {
...
let f_nueva_salida = (salida, empleado) => {
    return new Promise((resolve, reject) => {
        f_encuentra_entrada(empleado, salida)
            .then(duration => {
                Registro.findOneAndUpdate({ empleado: empleado, fin: false }, { salida, fin: true, jornada: duration },
                    (err, res) => {
                        if (err) console.log(err);
                        resolve({
                            res,
...
let f_busca_duplicado = (empleado, salida) => {
    return new Promise((resolve, reject) => {
        ...
        Registro.find({ empleado: empleado, entrada: { $gte: buscar_hoy } },
            (err, res) => {
                if (err) reject(err);
                console.log(res);
                resolve(res);
...
let f_avisa_empleado = (empleado_id) => {
    Usuario.findById(empleado_id, (err, res) => {
        console.log(res);
        enviar.f_manda_mensaje(res.telegram_id, `${res.alias} tienes un turno abierto, no olvides fichar la salida`)
    })
}
